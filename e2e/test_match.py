...
def test_match(page: Page, dev_server):
    ...
    name_input = test_page.filter_sidebar.get_input(name="name")
    name_input.type("Star Wars")

    match_selector = test_page.filter_sidebar.get_match_selector(name="name")
    ...
    assert "name__match=starts" in response_info.value.url
    assert "name=Star+Wars" in response_info.value.url
